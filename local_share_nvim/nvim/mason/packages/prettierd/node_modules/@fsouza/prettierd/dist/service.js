...
async function pluginSearchDirs(cwd, env) {
    const result = [];
...
async function tryToResolveConfigFromEnvironmentValue(prettier, editorconfig, value) {
    if (value) {
        return await prettier.resolveConfig((0, path_1.dirname)(value), {
            config: value,
            editorconfig,
            useCache: false,
        });
...
async function resolveConfigNoCache(env, prettier, filepath, { editorconfig = true }) {
    let config = await prettier.resolveConfig(filepath, {
        editorconfig,
        useCache: false,
    });
    if (!config) {
        config = await tryToResolveConfigFromEnvironmentValue(prettier, editorconfig, env.PRETTIERD_DEFAULT_CONFIG);
    }
    return config;
}
async function resolveConfig(env, prettier, filepath, options) {
...
async function resolvePrettier(env, filePath) {
    ...
    const cachedValue = caches.importCache.get(cacheKey);
    if (cachedValue) {
    ...
    return (_a = path, Promise.resolve().then(() => __importStar(require(_a)))).then((v) => {
        if (v !== undefined) {
...
async function run(cwd, { args, clientEnv }, text) {
    ...
    const resolvedPrettier = await resolvePrettier(env, path_2.default.dirname(fullPath));
    if (!resolvedPrettier) {
        return text;
    }
    ...
    const options = configPrecedence === "file-override" || configPrecedence === "prefer-file"
